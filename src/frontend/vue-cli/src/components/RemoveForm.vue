...
-->
    ...
    <div id="contact-form" style="z-index: 20">
        <div>
            <p style="color: #991D57; font-size: 24px">Вы уверены что хотите разбронировать место?
                <span class="glyphicon glyphicon-remove" @click="$store.state.remove = false"></span>
            </p>
        </div>
        ...
        <div class="form">
            <div>
                <label for="name">
                    ...
                    <input v-model="place.name" type="text" id="name" name="name" :value="getClient().name" :placeholder="getClient().name" required="required" tabindex="1" autofocus="autofocus" />
                </label>
            </div>
            <div>
                <label for="phone">
                    <span class="required">Телефон: *</span>
                    <input v-model="place.phone" type="text" id="phone" name="name" :value="getClient().phone" :placeholder="getClient().phone" required="required" tabindex="1" autofocus="autofocus" />
                </label>
            </div>
            <div>
                <label for="before">
                    <span>Время: </span>
                    <select v-model="time" id="before" name="subject" tabindex="4">
            ...
            <div>
                <button name="submit" type="submit" id="submit" v-on:click="onSubmit" >Снять бронь</button>
            </div>
        </div>
...
<script>
    export default {
        ...
        methods: {
            onSubmit(){
                let place = this.getClient(this.index)
                this.$http.post('reserve/remove', place, {
                }).then(
                    response => {
            ...
            getClient(){
                ...
                let time = parseInt(this.time.replace(':',''))
                let index = this.placeNumber
                 this.$store.state.places.forEach(function (place, i) {
                    if(place.number === index){
                        for(let j = 0; j < place.name.length; j++){

                            if((time >= parseInt(place['timeBefore'][j].replace(':',''))) && (time <= parseInt(place['timeAfter'][j].replace(':','')))){
                                client.name = place.name[j];
                                client.timeBefore = place.timeBefore[j];
                                client.timeAfter = place.timeAfter[j]
                                client.phone = place.phone[j]
                                client.number = index;
                               // console.log(typeof place.phone[j])
                            }
                        }
                    }
                })
