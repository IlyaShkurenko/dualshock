<template>
    <div>
        ...
        <item-cmp v-for="(item, index) in filteredRooms[page-1]" :event = "item" :index = "index" @click.native = "deleteRoom(item)"> </item-cmp>
        <a @click = "prevPage">Prev</a>
        <a @click = "nextPage">Next</a>
    </div>
</template>
<script>
    import Item from './Items/RemoveEventItem.vue'
    ...
    export default {
        data(){
            return {
                filteredText: '',
                selected: 'Id',
                fields: ['Id', 'Description', 'Category', 'Capacity', 'Status'],
                dash: [],
                page: 1,
                arrayRomms: []
            }
        ...
        methods:{
            ...
            nextPage(){
                if(this.page < this.events.length){
                    this.page++
                }
            },
            prevPage(){
        ...
        beforeCreate(){
            this.arrayRooms = this.$store.getters.events
        },
        props: {
            events: Array
        },
        ...
        computed:{
            returnArray(){
                if(this.events.length > 0){
                    return this.$store.state.events.filter(post => {
